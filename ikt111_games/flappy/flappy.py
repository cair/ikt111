...
class Flappy:
    ...
    def _update_display(self):
        ...
        for obstacle in self.obstacles:
            pygame.draw.rect(self.display, (obstacle.color), obstacle.rect)

        # Draws the preview of an user-created obstacle, if there is one
        if self.obstacle_preview is not None:
            self.obstacle_preview.update_rect(self.start_pos, pygame.mouse.get_pos())
            pygame.draw.rect(
                self.display, self.obstacle_preview.color, self.obstacle_preview.rect
            )

        # Draw goal
        pygame.draw.rect(self.display, colors["green"], self.goal)

        # Draw birds
        for bird in self.birds:
            pygame.draw.aalines(
                self.display, colors["black"], True, bird.real_points, True
            )
        ...
        try:
            ...
            pygame.draw.aalines(
                self.display, colors["red"], True, bird.real_points, True
            )
    ...
    def _check_mouse_event(self, e):
        ...
        if e.type == pygame.MOUSEBUTTONDOWN and e.button == 1:
            ...
            # Removes obstacle
                ...
                self.obstacles = [
                    o for o in self.obstacles if not o.rect.collidepoint(e.pos)
                ]
            self.obstacle_preview = None
        ...
        while True:
            ...
            while self.counter < self.max_life:
                ...
                self.stats["best_fitness"] = max(
                    self.birds, key=lambda bird: bird.fitness
                ).fitness
                self.stats["mean_fitness"] = sum(
                    [bird.fitness for bird in self.birds]
                ) / len(self.birds)
            ...
            time.sleep(
                1
            )  # Sleep 1 second just so we get a chance to read some stats at the end
            ...
            if not isinstance(self.birds, list):
                print(
                    f"Algorithm did not return a list! Got '{type(self.birds)}' instead"
                )
